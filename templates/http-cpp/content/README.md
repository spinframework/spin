...
* Run `wit-bindgen cpp ./wit/ --out-dir bindings -w http-trigger`

However, C++ bindings for WASI P3 and other async APIs are not yet available, so if you do need to regenerate bindings, it is currently recommended that you **NOT** do so from Spin `HEAD`.
