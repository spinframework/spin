...
use environment_definition::{
    load_environments, CompatibleWorld, CompatibleWorlds, TargetEnvironment2, TriggerType,
};
...
async fn validate_component_against_environments(
    ...
    for env in envs {
        ...
        if let Some(e) = validate_wasm_against_any_world2(env, worlds, component)
            .await
            .err()
        {
            errs.push(e);
...
async fn validate_wasm_against_any_world2(
    env: &TargetEnvironment2,
    worlds: &CompatibleWorlds,
    component: &ComponentToValidate<'_>,
) -> anyhow::Result<()> {
    let mut result = Ok(());
    for target_world in worlds {
        ...
        match validate_wasm_against_world2(env, target_world, component).await {
            Ok(()) => {
                tracing::info!(
...
async fn validate_wasm_against_world2(
    env: &TargetEnvironment2,
    target_world: &CompatibleWorld,
    component: &ComponentToValidate<'_>,
) -> anyhow::Result<()> {
