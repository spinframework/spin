...
/// The configuration settings allowed in the environment.
#[derive(Clone, Debug, Default, serde::Deserialize)]
#[serde(deny_unknown_fields)]
pub struct ConfigurationConstraints {
    /// Key-value stores allowed in the environment (None for no restrictions)
    pub key_value_stores: Option<Vec<String>>,
    /// SQLite databases allowed in the environment (None for no restrictions)
    pub sqlite_databases: Option<Vec<String>>,
    /// AI models allowed in the environment (None for no restrictions)
