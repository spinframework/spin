    ...
    for crate::PgFactorData<CF>
{
    ...
    async fn open_async<T>(
        ...
        Self::ensure_address_allowed_async(accessor, &address).await?;
    ...
    > {
        ...
        let (sr, efr) = accessor.with(|mut access| {
            let sr = StreamReader::new(&mut access, row_producer).map_err(|e| v4::Error::Other(format!("stream error {e}")))?;
            let efr = FutureReader::new(&mut access, error).map_err(|e| v4::Error::Other(format!("future error {e}")))?;
            Ok((sr, efr))
        })?;
