...
fn reassign_extra_components_from_triggers(mut locked: LockedApp) -> LockedApp {
    ...
    fn component_id(trigger: &LockedTrigger) -> Option<String> {
        trigger.trigger_config.get("component").and_then(|v| v.as_str()).map(|s| s.to_string())
    }
    fn set_component_id(app: &mut LockedApp, trigger_id: &str, component_id: &str) {
        let trigger = app.triggers.iter_mut().find(|t| t.id == trigger_id).unwrap();
        trigger.trigger_config.as_object_mut().unwrap().insert("component".into(), component_id.into());
    }
    fn extra_components(trigger: &LockedTrigger) -> Option<&ValuesMap> {
        trigger.trigger_config.get("components").and_then(|v| v.as_object())
    }
    fn has_extra_components(trigger: &LockedTrigger) -> bool {
        extra_components(trigger).is_some_and(|xcs| !xcs.is_empty())
    }
    fn triggers_referencing<'a>(all_triggers: &'a [LockedTrigger], cid: &String) -> Vec<&'a LockedTrigger> {
    ...
    for (cid, triggers) in needs_splitting {
        for trigger in &triggers {
            ...
            let mut component = locked.components.iter().find(|c| c.id == **cid).unwrap().clone();
        ...
        if let Some(extras) = extra_components(trigger) {
            if let Some(component_id) = component_id(trigger) {
                if let Some(component) = locked.components.iter_mut().find(|c| c.id == component_id) {
                    component.metadata.insert("trigger-extras".into(), extras.clone().into());
                    component.metadata.insert("resolve-extras-using".into(), trigger.trigger_type.clone().into());
                    trigger.trigger_config.as_object_mut().unwrap().remove("components");
                }
            }
        }
    }
...
mod test {
    ...
    async fn load_test_case(testcase_dir: &str, manifest_file: &str) -> anyhow::Result<(tempfile::TempDir, LockedApp)> {
        ...
        let locked_app = loader
            .load_file(app_root.join(manifest_file))
            .await;
    ...
    fn trigger_by_route<'a>(locked: &'a LockedApp, route: &str) -> &'a LockedTrigger {
        fn route_of(trigger: &LockedTrigger) -> &str {
            trigger.trigger_config.get("route").and_then(|v| v.as_str()).unwrap()
        }
        locked.triggers.iter().find(|t| route_of(t) == route).unwrap()
    }
    ...
    fn component_for_route<'a>(locked: &'a LockedApp, route: &str) -> &'a LockedComponent {
        ...
        locked.components.iter().find(|c| c.id == component_id).unwrap()
    }

    fn component_id(trigger: &LockedTrigger) -> &str {
        trigger.trigger_config.get("component").and_then(|v| v.as_str()).unwrap()
    }

    fn component_trigger_extras_for_route<'a>(locked: &'a LockedApp, route: &str, key: &str) -> &'a Vec<serde_json::Value> {
        let component = component_for_route(locked, route);
        let extras = component.metadata.get("trigger-extras").expect("should have had trigger-extras");
        extras.get(key).expect("should have had extras for key").as_array().expect("extras for key should have been an array")
    }

    #[tokio::test]
    async fn unenriched_lockfile_is_unchanged() {
        let (_wd, locked_app) = load_test_case("extra-components", "vanilla.toml").await.unwrap();
    ...
    async fn enriched_lockfile_only_one_trigger_per_component_no_changes() {
        let (_wd, locked_app) = load_test_case("extra-components", "inoffensive.toml").await.unwrap();
    ...
    async fn enriched_lockfile_multiple_enriched_triggers_per_component_get_split() {
        let (_wd, locked_app) = load_test_case("extra-components", "three-to-one.toml").await.unwrap();
        ...
        assert!(component_for_route(&locked_app, "/a1").source.content.source.as_ref().unwrap().ends_with("/a.dummy.wasm.txt"));
        assert_ne!("a", component_id(trigger_by_route(&locked_app, "/a2")));
        assert!(component_for_route(&locked_app, "/a3").source.content.source.as_ref().unwrap().ends_with("/a.dummy.wasm.txt"));
    ...
    async fn enriched_lockfile_captures_composition_graph_in_split_component() {
        ...
        assert_eq!("m1", a1_mw[0].as_str().expect("a1 mw should have been strings"));
        assert_eq!("m2", a1_mw[1].as_str().expect("a1 mw should have been strings"));

        let a2_mw = component_trigger_extras_for_route(&locked_app, "/a2", "middleware");
        assert_eq!(2, a2_mw.len());
        assert_eq!("m2", a2_mw[0].as_str().expect("a2 mw should have been strings"));
        assert_eq!("m3", a2_mw[1].as_str().expect("a2 mw should have been strings"));

        let a3_mw = component_trigger_extras_for_route(&locked_app, "/a3", "middleware");
        assert_eq!(3, a3_mw.len());
        assert_eq!("m3", a3_mw[0].as_str().expect("a3 mw should have been strings"));
        assert_eq!("m2", a3_mw[1].as_str().expect("a3 mw should have been strings"));
        assert_eq!("m1", a3_mw[2].as_str().expect("a3 mw should have been strings"));
        ...
        assert_eq!("m1", b_mw[0].as_str().expect("b mw should have been strings"));
        assert_eq!("m3", b_mw[1].as_str().expect("b mw should have been strings"));
    }
