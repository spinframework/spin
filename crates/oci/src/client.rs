...
impl Client {
    ...
    async fn assemble_layers_composed(
        ...
        let temp_dir =
            tempfile::tempdir().context("unable to create tempdir for precomposition")?;
        let working_dir = temp_dir.path();
        let locked_url = write_locked_app(&locked, working_dir)
            .await
...
async fn precompose_using_trigger(
    ...
    let Some(resolve_extras_using) = c
        .metadata
        .get("resolve-trigger-dependencies-using")
        .and_then(|v| v.as_str())
    else {
