...
`wasmtime_wasi_http` crates. So the standard pattern can't work.  However, Wasmtime 50 will
bring in named import support for the core WASI structures and traits. So `factor-wasi` and
`factor-outbound-http` will do things a little differently from other factors.

There remains an outstanding upstream issue, which currently means outbound HTTP stalls unless
the dependency uses a precisely matching version of `wit-bindgen`. The upstream devs are aware
of this and are looking into a fix.

## Other considerations

### Files

The current proof of concept allows a dependency to list files?  This appears to work
correctly locally, but will likely require further work on the OCI implementation.
